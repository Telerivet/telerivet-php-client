 ...
 * Read-only
 */
class TelerivetService extends TelerivetEntity
{
    /**
    ...
    public function invoke($options)
    {
        ...
        if ($invoke_result['sent_messages']) {
            ...
            foreach ($invoke_result['sent_messages'] as $sent_message_data) {
                $sent_messages[] = new TelerivetMessage($this->api, $sent_message_data);
            }
     ...
     */
    public function queryContactStates($options = null)
    {
     ...
     */
    public function getContactState($contact)
    {
        return new TelerivetContactServiceState($this->_api, $this->_api->doRequest('GET', $this->getBaseApiPath() . '/states/' . $contact->id));
    }
    ...
    Telerivet_ContactServiceState
     */
    public function setContactState($contact, $options)
    {
        return new TelerivetContactServiceState($this->_api, $this->_api->doRequest('POST', $this->getBaseApiPath() . '/states/' . $contact->id, $options));
    }
    ...
    Telerivet_ContactServiceState
     */
    public function resetContactState($contact)
    {
        return new TelerivetContactServiceState($this->_api, $this->_api->doRequest('DELETE', $this->getBaseApiPath() . '/states/' . $contact->id));
    }
